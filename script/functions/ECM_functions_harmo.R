...
ECM_eval_plot <- function(data, res_list, target_name = "log_end_snf", use_exp = TRUE) {
  ...
  p <- ggplot() +
    geom_line(
      data = all_preds,
      aes(x = t, y = value_fc, group = origin, color = origin),
      linewidth = 1
    ) +
    geom_line(
      data = df_real,
    ...
    scale_x_continuous(
      breaks = seq(floor(min(df_real$t)), 
                   ceiling(max(all_preds$t)), 
                   by = 1)
    ) +
    scale_y_continuous(
      limits = c(30, 90),
    ...
    labs(
      title = paste("Backtesting ECM —", target_name),
      subtitle = paste0(
        "RMSE moyen : ", round(mean(errors_rmse, na.rm = TRUE), 4),
        " | RMSPE moyen : ", round(mean(errors_rmspe, na.rm = TRUE), 2), "%",
