...
theme_backtest_harmonise <- function() {
  theme_backtest() +
    theme(
      ...
      panel.grid.minor = element_line(color = "grey92", linewidth = 0.20),
...
arima_expanding_test_plot <- function(data,
  ...
  for (end_train in seq(train_size, n - test_size, by = step)) {
    ...
    pred_list[[length(pred_list) + 1]] <- data.frame(
      ...
      lower = lower_pred,
      upper = upper_pred,
      window = paste0(format(time_vals[end_train], "%Y-%m"))  
    )
  }
  ...
  # --- Graphique ---
  # --- Modification du nom des variables pour l'affichage ---
  if (var_name == "endettement_menage") {
    var_name <- "Endettement des ménages"
  } else if (var_name == "endettement_snf") {
    var_name <- "Endettement des SNF"
  }
  ggplot() +
    geom_ribbon(
      data = pred_all,
    ...
    scale_x_date(
      limits = range(df_orig$time, na.rm = TRUE),
      breaks = seq(
        from = as.Date(format(min(df_orig$time, na.rm = TRUE), "%Y-01-01")),
        to   = as.Date(format(max(df_orig$time, na.rm = TRUE), "%Y-01-01")),
        by   = "1 year"
      ),
      date_labels = "%Y",
    ...
    scale_y_continuous(
      ...
      minor_breaks = seq(30, 90, by = 1.25)
    ) +
    labs(
      title = paste("Backtesting", ifelse(covid, "ARIMAX", "ARIMA"), "—", var_name),
      subtitle = paste("Expanding window – test =", test_size, "trimestres –", country),
      x = "Date",
      y = "Ratio d'endettement (% du PIB)",
      color = "Entraîné jusqu'à"
    ) +
    guides(color = guide_legend(nrow = 3)) +
    theme_backtest_harmonise()
}
